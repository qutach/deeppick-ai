...
def update_predictions(conn, updates: List[tuple]):
    if not updates:
        return
    # updates contain tuples (id, score)
    with conn.cursor() as cur:
        execute_values(
            cur,
            """
...
def predict_for_kommande(omgang_id: Optional[int], batch_size: int) -> None:
    ...
    with get_conn() as conn:
        ...
        while True:
            ...
            print(f"Predicter: uppdaterade {processed} rader... (senaste id={last_id})")

    print("Predicter: klart.")


def main():
    ap = argparse.ArgumentParser(description="Predicera rank-score till kommande.rank_predict_1 med tränad XGBRanker")
