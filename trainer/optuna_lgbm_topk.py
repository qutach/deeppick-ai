...
def build_label_gain(scheme: str, max_label: int) -> Optional[List[float]]:
    ...
    # Our labels live in {10, 11, 12, 13}. Define focused gains there.
    if scheme == "balanced":
...
def compute_recall_at_k(valid_df: pd.DataFrame, preds: np.ndarray, k: int) -> Dict[str, float]:
    # Group-level recalls: fraction of groups with at least one 13 in top-k,
    # and average recall for labels >=12 in top-k over groups that have >=12 labels.
    gids = valid_df["omgang_id"].values
    labels = valid_df["correct"].values

    # Build index per group
    ...
    # Append end index for iteration simplicity
    ...
    for s, e in zip(start_idx, end_idx):
        ...
        # Compute 13 presence recall (binary per group)
        denom13 = np.sum(lbls == 13)
        if denom13 > 0:
            groups_with_13 += 1
            if np.any(lbls[top_idx] == 13):
                groups_13_captured += 1
        # Compute >=12 recall per group
...
def objective_factory(df: pd.DataFrame, eval_at: List[int], early_stopping: int, seed: int,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        print(f"Trial {trial.number}: ndcg@{eval_at[-1]}={score:.6f} | recall13@{topk}={metrics['recall13@k']:.4f} | recall>=12@{topk}={metrics['recall_ge12@k']:.4f}")
...
def main():
    ap = argparse.ArgumentParser(description="Optuna hyperparameteroptimering för LGBMRanker (top-K fokus)")
    ap.add_argument("--trials", type=int, default=100, help="Antal Optuna-trials (default 100)")
    ...
    ap.add_argument("--eval-at", type=int, nargs="+", default=[500], help="eval_at nivåer för NDCG, sista används för Optuna-score")
    ...
    try:
        ...
        if args.save_model:
            ...
            # Save best params snapshot (+ gains and eval_at)
            snapshot = {
            ...
            with open("/models/lgbm_ranker_optuna_topk.json", "w", encoding="utf-8") as f:
                json.dump(snapshot, f, indent=2)

            print(
                f"Saved tuned model to {model_path} and params to /models/lgbm_ranker_optuna_topk.json"
            )
